...
def find_encoded_value(target_domain: str):
    for encoded in range(256, 65536):  # uint16 range from 256 to 65535
        result, _ = decode_tunnel_server_domain(encoded)
        if result == target_domain:
            return encoded
    return None

target_domain = "cable.i6khud6i7qci.com"
encoded_value = find_encoded_value(target_domain)

if encoded_value is not None:
    print(f"Encoded value for domain '{target_domain}': {encoded_value} (0x{encoded_value:04X})")
else:
    print(f"No matching encoded value found for '{target_domain}'")
